...
"""Finetune models on retrieval tasks (Mewsli-X).

Forked from run_retrieval_qa.py.
"""
...
from processors import mewslix
...
def train(args, train_dataset, model, tokenizer):
  ...
  if args.local_rank in [-1, 0]:
    tb_writer = SummaryWriter(comment="-" + os.path.basename(args.output_dir))
      ...
      if (step + 1) % args.gradient_accumulation_steps == 0:
        ...
        if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
          ...
          if args.local_rank == -1 and args.evaluate_during_training:
            logger.info("Start evaluate_during_training.")
            results = evaluate(args, model, tokenizer)
            for key, value in results.items():
              tb_writer.add_scalar("eval_{}".format(key), value, global_step)
            logger.info("End evaluate_during_training.")
...
def evaluate(args, model, tokenizer, prefix="", language='en', lang2id=None):
  dataset = load_and_cache_examples(
      args, tokenizer, evaluate=True, output_examples=False, language=language,
      lang2id=lang2id)
  ...
  for batch in tqdm(eval_dataloader, desc="Evaluating"):
    ...
    with torch.no_grad():
      ...
      # Unused, but could be used for computing in-batch eval metrics.
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False, language='en', lang2id=None):
  ...
  cached_features_file = os.path.join(
    input_dir,
    "cached_wikipedia_el_{}_{}_{}_{}".format(
  ...
  if os.path.exists(cached_features_file) and not args.overwrite_cache and not output_examples:
    ...
    if not args.data_dir and ((evaluate and not args.predict_file) or (not evaluate and not args.train_file)):
      ...
      processor = mewslix.WikiELProcessor()
      if evaluate:
        examples = processor.get_dev_examples(args.data_dir, filename=args.predict_file)
      else:
        examples = processor.get_train_examples(args.data_dir, filename=args.train_file)

    features, dataset = mewslix.convert_wikipedia_examples_to_features(
      examples=examples,
      tokenizer=tokenizer,
...
def main():
  ...
  if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
    ...
    logger.info("Reloading saved model from %s", args.output_dir)
